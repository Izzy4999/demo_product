...
export type PrizeType = "airtime" | "cash" | "draw";
...
interface Props {
  ...
  onPrizeAwarded?: (result: PromoResult) => void;
}

// ─── Prize copy ───────────────────────────────────────────────────────────────

const VERIFY_OK_WA =
  "✅ *GENUINE*\n\nThank you for buying this product!\n\nThis product is *authentic* and verified by Sproxil Verify™.\n_Location logged · now_";

const VERIFY_OK_SMS =
  "GENUINE: Your product is authentic. Verified by Sproxil. Thank you for buying!";

const PRIZE_WA: Record<PrizeType, string> = {
  airtime:
    "🎉 *Congratulations!*\n\nYou have won *₦500 Airtime* in the Sproxil Promo!\n\nAirtime will be credited to your number within 24 hours.\n\n_Keep buying to win more fantastic prizes!_ 🎊",
  cash:
    "🏆 *GRAND PRIZE WINNER!*\n\nYou have won *₦50,000 cash!*\n\nYour PAYCODE: *PAY-7723-XQRT*\nDial *737# to redeem to any bank account.\n\n_Congratulations from Team Sproxil!_ 🥳",
  draw:
    "🎟 *You're in the Monthly Grand Draw!*\n\nDraw Date: *30 Apr 2026*\nPrize Pool: *₦1,000,000*\nRef: *DRW-7829-KQ*\n\n_Keep buying for more chances to win!_ 🍀",
};

const PRIZE_SMS: Record<PrizeType, string> = {
  airtime:
    "CONGRATULATIONS! You won N500 Airtime in the Sproxil Promo! Will be credited within 24hrs. Keep buying to win more! -Sproxil",
  cash:
    "WINNER! You won N50,000 CASH in the Sproxil Promo! PAYCODE: PAY-7723-XQRT. Dial *737# to redeem to any bank. -Sproxil",
  draw:
    "You're in the Sproxil Grand Draw! Date: 30 Apr 2026. Prize Pool: N1,000,000. Ref: DRW-7829-KQ. Keep scanning to win! -Sproxil",
};

const NOTIF_LABEL: Record<PrizeType, string> = {
  airtime: "₦500 Airtime credited — Sproxil Promo",
  cash:    "₦50,000 win confirmed — Sproxil Promo",
  draw:    "Draw entry confirmed · Ref DRW-7829-KQ",
};
...
// ─── Status bar — exact copy from PhoneEmulator ───────────────────────────────
...
// ─── Boot screen — exact copy from PhoneEmulator ─────────────────────────────
...
// ─── Lock screen — same blue gradient, promo notifications ───────────────────
    ...
    >
      ...
      <div style={{ padding: "12px 14px 0", display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 10, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              38353: Send your code to enter the promo…
            </p>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, paddingBottom: 24, color: "rgba(255,255,255,0.45)" }}>
        <motion.div animate={{ y: [-3, 3, -3] }} transition={{ repeat: Infinity, duration: 1.4 }}
          onClick={onUnlock} style={{ cursor: "pointer" }}>
          <svg width={18} height={18} viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8z" />
          </svg>
        </motion.div>
        <p style={{ fontSize: 11, margin: 0, cursor: "pointer" }} onClick={onUnlock}>Tap to unlock</p>
      </div>
    </motion.div>
  );
}

// ─── App icon — exact copy from PhoneEmulator ────────────────────────────────
...
// ─── Home screen — exact copy from PhoneEmulator ─────────────────────────────
      ...
      <div style={{ padding: "10px 16px 12px" }}>
        <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 14, padding: "8px 14px", textAlign: "center" }}>
          <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 10, margin: 0 }}>
            Open <strong style={{ color: "white" }}>WhatsApp</strong> or <strong style={{ color: "white" }}>Messages</strong> to verify your code
          </p>
        </div>
      </div>
...
function ChatScreen({ app, prizeType, prefillCode, codeRevealed, onBack, onPrizeAwarded }: {
  app:            AppType;
  prizeType:      PrizeType;
  prefillCode:    string;
  codeRevealed:   boolean;
  ...
  const push = useCallback((from: "user" | "system", text: string) => {
    setMessages(prev => [...prev, { id: `${from}-${Date.now()}`, from, text, time: nowStr() }]);
  }, []);
  ...
  const sendMessage = useCallback(() => {
    ...
    // Step 1: genuine verify response
    setTimeout(() => {
      const verifyMsg = isWa ? VERIFY_OK_WA : VERIFY_OK_SMS;
      push("system", verifyMsg);
      setWaiting(false);

      // Step 2: prize response (after short pause)
      setTimeout(() => {
        setWaiting(true);
        setTimeout(() => {
          const prizeMsg = isWa ? PRIZE_WA[prizeType] : PRIZE_SMS[prizeType];
          push("system", prizeMsg);
          setWaiting(false);
          if (!prizeSent) {
            setPrizeSent(true);
            onPrizeAwarded();
  ...
  }, [inputText, waiting, isWa, prizeType, prizeSent, push, onPrizeAwarded]);
      ...
      <div style={{ background: headerBg, padding: "8px 10px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ color: "#a7f3d0", fontSize: 10, margin: 0, marginTop: 1 }}>
            {isWa ? "online · Sproxil Promo" : "Sproxil Promo Service"}
          </p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
        ...
        {messages.length === 0 && (
          <div style={{ display: "flex", justifyContent: "center", marginTop: 12 }}>
            <div style={{ background: "#fff8dc", border: "1px solid #e6d88a", borderRadius: 12, padding: "10px 14px", fontSize: 11, color: "#7a5c00", textAlign: "center", maxWidth: "85%" }}>
              <p style={{ fontWeight: 700, marginBottom: 4, margin: "0 0 4px" }}>Sproxil Promo™</p>
              <p style={{ margin: 0 }}>Type the code from your product&apos;s scratch label and send it to verify and enter the prize draw.</p>
            </div>
          </div>
        )}
...
export default function PromoPhoneEmulator({ prizeType, prefillCode, codeRevealed, onPrizeAwarded }: Props) {
  const [phase, setPhase]         = useState<Phase>("off");
  const [app, setApp]             = useState<AppType | null>(null);
  const [time, setTime]           = useState("9:41");
  const [prizeDone, setPrizeDone] = useState(false);
  // notif: null = hidden, "whatsapp"|"sms" = showing for that channel
  const [notifFor, setNotifFor]   = useState<AppType | null>(null);
  const bootTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Reset when prize scenario changes
  useEffect(() => {
    setPhase("off");
    setApp(null);
    setPrizeDone(false);
  ...
  // Called by ChatScreen when prize messages have been sent
  const handlePrizeAwarded = useCallback(() => {
    if (prizeDone) return;
    setPrizeDone(true);
    // Fire the page-level callback
    onPrizeAwarded?.({ prizeType, code: prefillCode, channel: app ?? "whatsapp" });
    // Prize notification always comes in as SMS regardless of verify channel
    setTimeout(() => {
      setNotifFor("sms");
      // Auto-hide after 5 s
      setTimeout(() => setNotifFor(null), 5000);
    }, 1400);
  }, [prizeDone, prizeType, prefillCode, app, onPrizeAwarded]);

  // Clicking the notification opens the chat for that channel
  ...
  return (
    <div style={{
      ...
      {/* Prize notification banner — clickable */}
      <AnimatePresence>
        {notifFor && (
          <motion.button
            key="notif"
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 9, margin: 0, textTransform: "uppercase", letterSpacing: 0.5 }}>
                {notifFor === "whatsapp" ? "WhatsApp" : "Messages"} · now
              </p>
              <p style={{ color: "white", fontSize: 12, fontWeight: 700, margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {PRIZE_ICON[prizeType]} {NOTIF_LABEL[prizeType]}
              </p>
            </div>
            {/* Tap hint */}
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative", cursor: phase === "off" ? "pointer" : "default" }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "chat" && app && (
            <motion.div key={`chat-${app}-${prizeType}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ width: "100%", height: "100%" }}>
              <ChatScreen
                app={app}
                prizeType={prizeType}
                prefillCode={codeRevealed ? prefillCode : ""}
