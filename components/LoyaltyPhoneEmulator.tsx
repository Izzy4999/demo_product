...
const COLOR    = "#7B2FBE";   // Loyalty purple
...
function LockScreen({ time, onUnlock, onOpenApp }: {
  ...
  return (
    <motion.div
      ...
      style={{ width: "100%", height: "100%", background: "linear-gradient(170deg,#0f172a 0%,#1e3a8a 55%,#0f172a 100%)", display: "flex", flexDirection: "column", userSelect: "none" }}
    >
      {/* Notification cards */}
...
function ChatScreen({ app, code, codeRevealed, onBack, onPointsAwarded }: {
  ...
  const headBg   = isWa ? WA_DARK : "#1e293b";
    ...
    <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
      ...
      <div style={{ background: "#f0f0f0", borderTop: "1px solid #e5e7eb", padding: "8px 10px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        ...
        <button onClick={send} onMouseDown={e => e.preventDefault()} disabled={!inputText.trim() || waiting || done}
          style={{ width: 40, height: 40, borderRadius: 9999, border: "none", background: isWa ? WA_GREEN : "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", cursor: inputText.trim() && !waiting && !done ? "pointer" : "not-allowed", opacity: inputText.trim() && !waiting && !done ? 1 : 0.45, flexShrink: 0, transition: "opacity 0.2s" }}>
          <SendIcon />
        </button>
      </div>
